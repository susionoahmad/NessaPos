...
func migrate() {
	...
	_, _ = DB.Exec("ALTER TABLE settings ADD COLUMN bridge_token TEXT DEFAULT ''")
	_, _ = DB.Exec("ALTER TABLE settings ADD COLUMN bridge_port INTEGER DEFAULT 12348")
	_, _ = DB.Exec("ALTER TABLE settings ADD COLUMN allowed_origins TEXT DEFAULT '*'")

	migrateOrderItemsTable()
}

func migrateOrderItemsTable() {
	row := DB.QueryRow("SELECT sql FROM sqlite_master WHERE type='table' AND name='order_items'")
	...
	if strings.Contains(createSQL, "FOREIGN KEY (product_id)") || strings.Contains(createSQL, "product_id INTEGER NOT NULL") {
		_, _ = DB.Exec("ALTER TABLE order_items RENAME TO order_items_old")
		_, _ = DB.Exec(`
		...
		`)
		_, _ = DB.Exec(`
		INSERT INTO order_items (id, order_id, product_id, product_name, quantity, price, discount, total, created_at, updated_at)
		SELECT id, order_id, product_id, product_name, quantity, price, discount, total, created_at, updated_at
		FROM order_items_old
		`)
		_, _ = DB.Exec("DROP TABLE order_items_old")
	}
}
...
func createTables() {
	...
	CREATE TABLE IF NOT EXISTS settings (
		...
		bridge_token TEXT DEFAULT '',
		bridge_port INTEGER DEFAULT 12348,
...
func seedData() {
	...
	if count == 0 {
		DB.Exec("INSERT INTO settings (store_name, store_address, store_phone, tax_rate, tax_type, receipt_text, printer_name, refresh_interval_sec, print_session_slip, trial_start, last_run, license_blob, bridge_token, bridge_port, allowed_origins) VALUES ('SmartPOS Store', 'Jl. Merdeka No 1', '08123456789', 10, 'exclusive', 'Terima kasih atas kunjungan Anda', '', 30, 1, '', '', '', '', 12348, '*')")
	}
