    ...
    <div class="modal-content">
      ...
      <div class="stats-grid">
        <div class="stat-item">
          ...
          <div class="val">Rp {{ session.start_amount.toLocaleString() }}</div>
        </div>
        <div class="stat-item">
          <label>Penjualan Tunai</label>
          <div class="val">Rp {{ sales.toLocaleString() }}</div>
        </div>
        <div class="stat-item highlight">
          <label>Saldo Seharusnya</label>
          <div class="val">Rp {{ (session.start_amount + sales).toLocaleString() }}</div>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <input 
          type="text" 
          :value="'Rp ' + physicalAmount.toLocaleString()" 
          readonly 
          class="total-display"
          :class="{ 'text-danger': difference < 0, 'text-success': difference > 0 }"
        />
        <div v-if="difference !== 0" class="diff-label">
          Selisih: <strong :class="difference < 0 ? 'text-danger' : 'text-success'">
...
const difference = computed(() => {
  const expected = props.session.start_amount + props.sales
