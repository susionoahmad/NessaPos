<template>
  <div class="modal-overlay">
    <div class="modal-content">
      <h2>Buka Sesi Kasir</h2>
      <p class="subtitle">Input rincian modal awal di laci kasir</p>

      <div class="vault-info">
        Saldo Brankas: <strong>Rp {{ vaultBalance.toLocaleString() }}</strong>
      </div>

      <div class="form-group">
        <label>Total Modal Awal (Rp)</label>
        <input 
          type="text" 
      ...
      <div class="denom-container">
        ...
        <div class="denom-section">
          ...
          <div v-for="(denom, index) in paperDenoms" :key="'p'+denom.val" class="denom-row">
            <label>Rp {{ denom.val.toLocaleString() }}</label>
      ...
      <div class="actions">
        ...
        <button 
          class="btn-confirm" 
          @click="handleOpen" 
          :disabled="totalAmount <= 0 || totalAmount > vaultBalance"
        >
          Buka Sesi (Cetak Slip)
        </button>
      </div>
    </div>
  </div>
...
const focusNext = (type: string, index: number) => {
  if (type === 'p') {
    ...
    if (index < coinDenoms.value.length - 1) {
      ...
      if (totalAmount.value > 0 && totalAmount.value <= props.vaultBalance) {
        handleOpen()
      }
    }
  }
...
const handleOpen = () => {
  ...
  coinDenoms.value.forEach(d => {
      // Merge 1000 if exists in both, though usually we'd keep them separate in UI
...
.btn-confirm { background: #0ea5e9; color: white; box-shadow: 0 4px 6px -1px rgba(14, 165, 233, 0.4); }
