    ...
    <div class="report-layout">
      ...
      <aside class="report-sidebar">
        <div class="menu-section">
          ...
          <div class="menu-items" v-if="openMenus.sales">
            <div 
              class="menu-item" 
        ...
        <div class="menu-section">
          ...
          <div class="menu-items" v-if="openMenus.finance">
            <div 
              class="menu-item" 
        ...
        <div class="menu-section">
          ...
          <div class="menu-items" v-if="openMenus.inventory">
            <div 
              class="menu-item" 
        ...
        <div class="content-area">
              ...
              <table class="report-table">
                ...
                <tbody>
                  <tr v-for="t in filteredCashier" :key="t.id">
                    ...
                    <td class="text-right text-success">{{ t.amount > 0 ? 'Rp ' + t.amount.toLocaleString() : '-' }}</td>
                    <td class="text-right text-danger">{{ t.amount < 0 ? 'Rp ' + Math.abs(t.amount).toLocaleString() : '-' }}</td>
                    <td class="text-right font-bold">Rp {{ t.balance_after?.toLocaleString() }}</td>
                  </tr>
          ...
          <div v-if="activeTab === 'tax-report'" class="tab-content">
            <div class="table-card">
              <div class="tc-header">
                ...
                <div class="count">Total Pajak: Rp {{ totalTax.toLocaleString() }}</div>
              </div>
              <table class="report-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr v-for="order in filteredSales" :key="order.id">
                    ...
                    <td class="text-right">Rp {{ order.final_amount.toLocaleString() }}</td>
                    <td class="text-right font-bold text-slate">Rp {{ (order.tax_amount || 0).toLocaleString() }}</td>
                  </tr>
                ...
                <tfoot v-if="filteredSales.length > 0">
                  <tr style="background: #f8fafc;">
                    ...
                    <td class="text-right font-bold text-success" style="font-size: 16px;">Rp {{ totalTax.toLocaleString() }}</td>
                  </tr>
                </tfoot>
              </table>
                  ...
                  <div class="order-header" @click="toggleOrder(order.id)">
                    ...
                    <div class="oh-right">
                      <div class="oh-tax-info" v-if="order.tax_amount > 0">
                        ...
                        <span class="tax-value">Rp {{ order.tax_amount.toLocaleString() }}</span>
                      </div>
                      <span class="order-total">Rp {{ order.final_amount.toLocaleString() }}</span>
                    </div>
                  </div>
                    ...
                    <table class="detail-table">
                      ...
                      <tfoot>
                        <tr>
                          ...
                          <td class="text-right">Rp {{ order.total_amount.toLocaleString() }}</td>
                        </tr>
                        <tr v-if="order.discount > 0">
                          <td colspan="3" class="text-right text-orange">Diskon</td>
                          <td class="text-right text-orange">-Rp {{ order.discount.toLocaleString() }}</td>
                        </tr>
                        <tr v-if="order.tax_amount > 0">
                          <td colspan="3" class="text-right">Pajak (Service)</td>
                          <td class="text-right">+Rp {{ order.tax_amount.toLocaleString() }}</td>
                        </tr>
          ...
          <div v-if="activeTab === 'vault'" class="tab-content">
            <div class="vault-header card bg-dark">
                ...
                <div class="vault-balance">Rp {{ vault.balance?.toLocaleString() }}</div>
              ...
              <table class="report-table">
                ...
                <tbody>
                  <tr v-for="s in filteredSessions" :key="s.id">
                    ...
                    <td class="text-right">
                      <div class="text-small">M: Rp {{ (s.start_amount || 0).toLocaleString() }}</div>
                      <div class="val" v-if="s.end_amount_physical">F: Rp {{ (s.end_amount_physical || 0).toLocaleString() }}</div>
                    </td>
                    <td class="text-right font-bold" :class="(s.difference || 0) < 0 ? 'text-danger' : 'text-success'">
                       {{ (s.difference || 0) > 0 ? '+' : '' }}{{ s.difference ? 'Rp ' + s.difference.toLocaleString() : '-' }}
                    </td>
                  </tr>
          ...
          <div v-if="activeTab === 'profit-loss'" class="tab-content">
            <div class="stats-row" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;">
                <div class="stat-box card">
                    ...
                    <div class="val text-success">Rp {{ totalRevenue.toLocaleString() }}</div>
                </div>
                <div class="stat-box card">
                    <label>Total HPP (Modal)</label>
                    <div class="val text-danger">Rp {{ totalCOGS.toLocaleString() }}</div>
                </div>
                <div class="stat-box card Highlight">
                    <label>Estimasi Laba</label>
                    <div class="val text-profit">Rp {{ (totalRevenue - totalCOGS).toLocaleString() }}</div>
                </div>
                <div class="stat-box card" style="background: #f8fafc;">
                    <label>Pajak Terkumpul</label>
                    <div class="val" style="color: #64748b;">Rp {{ totalTax.toLocaleString() }}</div>
                </div>
            </div>
              ...
              <table class="report-table">
                ...
                <tbody>
                  <tr v-for="s in profitLossData" :key="s.id">
                    ...
                    <td class="text-right">Rp {{ s.revenue.toLocaleString() }}</td>
                    <td class="text-right text-dim">Rp {{ s.cogs.toLocaleString() }}</td>
                    <td class="text-right font-bold text-success">Rp {{ (s.revenue - s.cogs).toLocaleString() }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <table class="report-table">
                ...
                <tbody>
                  <tr v-for="(p, idx) in fastMovingData" :key="p.id">
                    ...
                    <td class="text-right text-success">Rp {{ p.totalRevenue.toLocaleString() }}</td>
                  </tr>
...
import { 
    GetVault, GetVaultTransactions, AddCapital, 
    GetAllSessions, GetUsers, GetDetailedSales, GetCashierTransactions,
    GetStockMutations, GetProducts
...
const loadData = async () => {
    if (auth.user?.role === 'admin') {
        const [v, vt, ct, ss, us, sd, sm, ps] = await Promise.all([
            GetVault(), GetVaultTransactions(), GetCashierTransactions(), 
            GetAllSessions(), GetUsers(), GetDetailedSales(),
            GetStockMutations(), GetProducts()
        ])
        vault.value = v || {}
        vaultTransactions.value = vt || []
        cashierTransactions.value = ct || []
        sessions.value = ss || []
        users.value = us || []
        salesData.value = sd || []
        stockMutations.value = sm || []
        products.value = ps || []
    }
}
...
const getProductHpp = (productId: number) => {
    ...
    // product costs at the time of sale (e.g., using average cost, FIFO, LIFO).
    // For now, we'll use the current cost_price.
...
const profitLossData = computed(() => {
    return filteredSales.value.map(s => ({
        id: s.id,
        date: s.created_at,
        revenue: s.final_amount - s.tax_amount, // Use net revenue excluding tax
...
const productSalesStats = computed(() => {
    ...
    // Initialize with all products to include zero sales
...
const handleAddCapital = async () => {
    try {
        await AddCapital(capitalAmount.value, auth.user.id, capitalDesc.value)
...
const handlePrint = () => {
  ...
  // Give Vue more time to render all items (especially expanded details)
  setTimeout(() => {
    window.print()
