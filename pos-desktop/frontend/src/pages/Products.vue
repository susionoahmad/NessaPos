      ...
      <table>
        ...
        <tbody>
          <tr v-for="p in products" :key="p.id">
            ...
            <td class="text-dim">Rp {{ (p.cost_price || 0).toLocaleString() }}</td>
            <td class="text-primary">Rp {{ (p.selling_price || 0).toLocaleString() }}</td>
        ...
        <div class="modal-content text-center">
            ...
            <div class="import-stats" v-if="importResult.success">
                <div class="stat-item green">
                    <span class="stat-num">{{ importResult.count }}</span>
...
import { GetProducts, CreateProduct, UpdateProduct, UpdateProductStock, DeleteProduct, RecordStockMutation, ImportExcel } from '../../wailsjs/go/api/API'
...
const saveProduct = async () => {
  try {
    if (isEdit.value) {
      await UpdateProduct(form.value as any)
    } else {
...
const processMutation = async () => {
    ...
    if (mutateType.value === 'G->R') {
        ...
        fromLoc = "RAK"; toLoc = "GUDANG"
        p.shelf_stock -= mutateQty.value
        p.warehouse_stock += mutateQty.value
    } else if (mutateType.value === 'SUP->G') {
        fromLoc = "SUPPLIER"; toLoc = "GUDANG"; mType = "PURCHASE"
        p.warehouse_stock += mutateQty.value
    }

    try {
        await UpdateProductStock(p)
        await RecordStockMutation({
            product_id: p.id,
            type: mType,
            from_location: fromLoc,
            to_location: toLoc,
            quantity: mutateQty.value,
            reference: "Manual Mutation",
...
const handleExcelImport = async () => {
    ...
    try {
        const count = await ImportExcel()
        await loadData()
        importResult.value = { success: true, count }
    } catch(e: any) {
        const msg = String(e)
        if (msg.includes('CANCELLED')) {
            // user closed file dialog — silently ignore
            return
        }
        importResult.value = {
            success: false,
