  ...
  <div class="page-container">
    ...
    <div class="stats-grid">
      <div class="stat-card">
        ...
        <div class="value">Rp {{ daily.toLocaleString() }}</div>
        <router-link to="/reports" class="link">Lihat Laporan</router-link>
      </div>
      <div class="stat-card">
        <div class="label">Saldo Brankas</div>
        <div class="value">Rp {{ vaultBalance.toLocaleString() }}</div>
      ...
      <div class="stat-card">
        ...
        <div class="value">Rp {{ monthly.toLocaleString() }}</div>
...
import { GetDailySales, GetMonthlySales, GetVault, GetProducts } from '../../wailsjs/go/api/API'
...
onMounted(async () => {
  try {
    daily.value = await GetDailySales() || 0
    monthly.value = await GetMonthlySales() || 0
    const v = await GetVault()
    vaultBalance.value = v?.balance || 0
    const p = await GetProducts()
    productCount.value = p?.length || 0
  } catch (e) {
