...
func (r *OrderRepository) CreateTransaction(order models.Order, items []models.OrderItem, payment models.Payment) error {
	...
	for _, item := range items {
		_, err := tx.Exec("INSERT INTO order_items (order_id, product_id, product_name, quantity, price, discount, total) VALUES (?, ?, ?, ?, ?, ?, ?)",
			orderID, item.ProductID, item.ProductName, item.Quantity, item.Price, item.Discount, item.Total)
		if err != nil {
			tx.Rollback()
			return err
		}
...
func (r *OrderRepository) GetDailyCashSales(date string) (float64, error) {
	...
	query := `
		...
		WHERE date(o.created_at, 'localtime') = ? AND LOWER(p.payment_method) = 'cash'
	`
	err := r.DB.QueryRow(query, date).Scan(&total)
	return total, err
}
...
func (r *OrderRepository) GetDailyPaymentCounts(date string) (int, int, error) {
	return r.GetFilteredPaymentCounts(date, 0, "", "")
}

func (r *OrderRepository) GetFilteredPaymentCounts(date string, userID int, startTime string, endTime string) (int, int, error) {
	query := `
		SELECT 
			COALESCE(SUM(CASE WHEN LOWER(p.payment_method) = 'cash' THEN 1 ELSE 0 END), 0) AS cash_count,
			COALESCE(SUM(CASE WHEN LOWER(p.payment_method) != 'cash' THEN 1 ELSE 0 END), 0) AS non_cash_count
		FROM orders o
		JOIN payments p ON o.id = p.order_id
	...
	err := r.DB.QueryRow(query, date, userID, userID, startTime, startTime, endTime, endTime).Scan(&cashCount, &nonCashCount)
